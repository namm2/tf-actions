...
for d in set(changes):	
	...
	print('Running "terraform fmt" ...', subprocess.run(['terraform', 'fmt', '-diff=true'], capture_output=True))
	
	print('Running "terraform init" ...', subprocess.run(['terraform', 'init'], capture_output=True))
	
	print('Running "terraform validate" ...', subprocess.run(['terraform', 'validate'], capture_output=True))
